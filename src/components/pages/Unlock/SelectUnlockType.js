...
class SelectUnlockType extends Component {
  ...
  _buildList = () => {
    ...
    return (
      <Menu onSelect={this._onSelect}>
        {
          withSearchBox &&
          (<MenuItem disabled={true} className="search-box" >
            <Icon type="search" />
            <input autoFocus={true} placeholder="Search Asset" onChange={this._onSearch} />
          </MenuItem>)
        }
        {items}
      </Menu>
  ...
  _buildDefaultValue = () => {
    ...
    return 0 === options.length ? "" : options[0].render ? options[0].render() : options[0].text
  };
